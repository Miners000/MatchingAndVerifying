...
public:
    StableMatching(int size) : n(size), proposalCount(0)
    {
    ...
    void readInput(istream& input)
    {
        ...
        for(int j = 0; j < n; j++)
        {
            ...
            {
                int hospital = studentPrefs[j][rank];
                studentRanking[j][hospital] = rank;
            }
        }
    }
...
int main(int argc, char* argv[])
{
    ...
    cerr << "n = " << n << ", ";
    cerr << "Proposals = " << matching.getProposalCount() << ", ";
    cerr << "Time = " << duration.count() << " microseconds" << endl;
